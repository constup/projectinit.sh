...
setup_dev_entrypoint() {
  cp -f "${tool_dir}/src/language/php/symfony/container/docker/entrypoint/v1/template/dev" "${project_root_dir}/start_dev.sh"
  if [ "${projectinit_database_type}" = "no database" ]; then
    perl -i -ne 'print unless /~~~database creation and migration~~~/;' "${project_root_dir}/start_dev.sh"
  else
    perl -i -pe "s/~~~database creation and migration~~~/$(<"${tool_dir}/src/language/php/symfony/container/docker/entrypoint/v1/template/database" perl -pe 's/([\/\& \t])/\\$1/g')/g" "${project_root_dir}/start_dev.sh"
  fi
}

setup_prod_entrypoint() {
  cp -f "${tool_dir}/src/language/php/symfony/container/docker/entrypoint/v1/template/prod" "${project_root_dir}/start.sh"
  if [ "${projectinit_database_type}" = "no database" ]; then
    perl -i -ne 'print unless /~~~database creation and migration~~~/;' "${project_root_dir}/start.sh"
  else
    perl -i -pe "s/~~~database creation and migration~~~/$(<"${tool_dir}/src/language/php/symfony/container/docker/entrypoint/v1/template/database" perl -pe 's/([\/\& \t])/\\$1/g')/g" "${project_root_dir}/start.sh"
  fi
}
