...
copy_project_templates() {
  ...
  cp -f "${PROJECTINIT_DOCKER_TEMPLATE_DOCKER_COMPOSE}" "${project_root_dir}/compose_dev.yaml"
...
inject_project_service_properties() {
  ...
  echo "docker service name dev..."
  perl -pi -e "s/~~~service name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}/compose_dev.yaml"
  echo "container name dev..."
  perl -pi -e "s/~~~container name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}/compose_dev.yaml"
  echo "docker service name prod..."
  perl -pi -e "s/~~~service name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}/compose.yaml"
  echo "container name prod..."
  perl -pi -e "s/~~~container name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}/compose.yaml"
  echo "host port dev..."
  perl -pi -e "s/~~~host port~~~/${PROJECTINIT_DOCKER_APP_HOST_PORT}/g" "${project_root_dir}/compose_dev.yaml"
  echo "host port prod..."
  perl -pi -e "s/~~~host port~~~/${PROJECTINIT_DOCKER_APP_HOST_PORT}/g" "${project_root_dir}/compose.yaml"
  echo "symfony version..."
  perl -pi -e "s/~~~framework version~~~/${PROJECTINIT_DOCKER_INPUT_FRAMEWORK_VERSION}/g" "${project_root_dir}/start.sh"
  ...
  echo "language version for dev..."
  perl -pi -e "s/~~~language version~~~/${PROJECTINIT_DOCKER_PROJECT_LANGUAGE_VERSION}/g" "${project_root_dir}/Dockerfile_dev"
  echo "language version for prod..."
  perl -pi -e "s/~~~language version~~~/${PROJECTINIT_DOCKER_PROJECT_LANGUAGE_VERSION}/g" "${project_root_dir}/Dockerfile"
  perl -pi -e "s/      dockerfile: Dockerfile_dev/      dockerfile: Dockerfile/g" "${project_root_dir}/compose.yaml"
