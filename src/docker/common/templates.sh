...
copy_installer_templates() {
  ...
  echo "docker compose..."
  cp "${PROJECTINIT_DOCKER_INSTALLER_TEMPLATE_DOCKER_COMPOSE}" "${project_root_dir}/docker-compose.yml"
  echo "dockerfile..."
...
cleanup_installer_templates() {
  ...
  rm "${project_root_dir}/docker-compose.yml" "${project_root_dir}/Dockerfile" "${project_root_dir}/start.sh"
...
copy_project_templates() {
  ...
  echo "docker compose..."
  cp -f "${PROJECTINIT_DOCKER_TEMPLATE_DOCKER_COMPOSE}" "${project_root_dir}/docker-compose.yml"
  echo "dockerfile..."
...
inject_installer_properties() {
  ...
  perl -pi -e "s/~~~service name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}"/docker-compose.yml
  echo "container name..."
  perl -pi -e "s/~~~container name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}"/docker-compose.yml
  echo "host port..."
  perl -pi -e "s/~~~host port~~~/${PROJECTINIT_DOCKER_APP_HOST_PORT}/g" "${project_root_dir}"/docker-compose.yml
...
inject_project_service_properties() {
  ...
  perl -pi -e "s/~~~symfony version~~~/${PROJECTINIT_DOCKER_INPUT_SYMFONY_VERSION}/g" "${project_root_dir}"/start.sh
  echo "docker image name..."
  perl -pi -e "s/~~~image name~~~/${PROJECTINIT_DOCKER_PROJECT_SERVICE_NAME}/g" "${project_root_dir}"/docker-compose.yml
  echo "language version..."
...
cleanup_project_templates() {
  ...
  if [ "$PROJECTINIT_DOCKER_SERVICE_DEPENDENCIES" -eq 0 ]; then
    ...
    perl -i -ne 'print unless /depends_on:/;' "${project_root_dir}/docker-compose.yml"
  fi
  if [ -z "$PROJECTINIT_DOCKER_DATABASE_SERVICE_NAME" ]; then
    echo "database service in docker-compose.yml"
    perl -i -ne 'print unless /~~~database service~~~/;' "${project_root_dir}/docker-compose.yml"
