...
configure_phpunit() {
  ...
  if [ "$projectinit_use_projectinit_phpunit_dist" -eq 1 ]; then
    case $projectinit_phpunit_version in
      "phpunit/phpunit:^9" )
        cp -f "${tool_dir}/src/new/language/php/tools/unit_testing/phpunit/v9.xml.dist" "${project_root_dir}/phpunit.xml.dist"
        ;;
      "phpunit/phpunit:^10" )
        cp -f "${tool_dir}/src/new/language/php/tools/unit_testing/phpunit/v10.xml.dist" "${project_root_dir}/phpunit.xml.dist"
        ;;
      "phpunit/phpunit:^11" )
        cp -f "${tool_dir}/src/new/language/php/tools/unit_testing/phpunit/v11.xml.dist" "${project_root_dir}/phpunit.xml.dist"
        ;;
      "phpunit/phpunit:^12" )
        cp -f "${tool_dir}/src/new/language/php/tools/unit_testing/phpunit/v12.xml.dist" "${project_root_dir}/phpunit.xml.dist"
        ;;
    esac
  fi
